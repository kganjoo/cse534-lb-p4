...
control MyIngress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    apply{
        ...
        if (hdr.ipv4.isValid() && hdr.tcp.isValid()){
            ...
            //if current port is not equal to previous stored client port and state = 1.
            //1. update stored client port register, 
            //2 toggle the state variable

            if(standard_metadata.ingress_port == 0 && state1==1 ){
